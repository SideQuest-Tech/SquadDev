...
const DotField = memo(({
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      gradient.addColorStop(0, p.gradientFrom ?? 'rgba(168, 85, 247, 0.35)')
      gradient.addColorStop(1, p.gradientTo ?? 'rgba(180, 151, 207, 0.25)')
      ctx.fillStyle = gradient
      const cursorRadiusSq = (p.cursorRadius ?? 500) * (p.cursorRadius ?? 500)
      const radius = (p.dotRadius ?? 1.5) / 2
      ctx.beginPath()
      ...
      for (let i = 0; i < dots.length; i++) {
        ...
        if (distSq < cursorRadiusSq && engaged > 0.01) {
          ...
          if (p.bulgeOnly) {
            const proximity = 1 - dist / (p.cursorRadius ?? 500)
            const push = proximity * proximity * (p.bulgeStrength ?? 67) * engaged
        ...
        if ((p.waveAmplitude ?? 0) > 0) {
          drawY += Math.sin(dot.ax * 0.03 + t) * (p.waveAmplitude ?? 0)
          drawX += Math.cos(dot.ay * 0.03 + t * 0.7) * (p.waveAmplitude ?? 0) * 0.5
        }
