...
const fmtDateTime = (iso: string): string => new Date(iso).toLocaleString('en-ZA', {
  weekday: 'short', day: '2-digit', month: 'short',
  year: 'numeric', hour: '2-digit', minute: '2-digit', timeZone: TZ
})

const fmtDate = (iso: string): string => new Date(iso).toLocaleDateString('en-ZA', {
  day: '2-digit', month: 'short', timeZone: TZ
})

const fmtTime = (iso: string): string => new Date(iso).toLocaleTimeString('en-ZA', {
...
const gcalUrl = (meeting: Meeting): string => {
...
const downloadICS = (meeting: Meeting): void => {
  const s = new Date(meeting.confirmedTime!)
  const e = new Date(s.getTime() + meeting.duration * 60000)
...
function RequestModal({ onClose, onCreated }: RequestModalProps): React.ReactElement {
...
function AcceptModal({ meeting, onClose, onDone }: AcceptModalProps): React.ReactElement {
...
function MeetingCard({ meeting, onRespond }: MeetingCardProps): React.ReactElement {
...
export default function MeetingsTab({ clientName, clientCompany }: MeetingsTabProps): React.ReactElement {
  const [meetings, setMeetings] = useState<Meeting[]>([])
  const [loading, setLoading] = useState(true)
  const [showRequest, setShowRequest] = useState(false)
  const [respondTo, setRespondTo] = useState<Meeting | null>(null)
  ...
  const load = () => {
    ...
    fetch('/api/meetings', {
      ...
      .catch((err: Error) => logger.error('Failed to load meetings', { message: err.message }))
