...
const statusClass = (s: string): string =>
  ({ in_progress: 'in-progress', paused: 'paused', cancelled: 'cancelled', completed: 'completed' }[s] || 'in-progress')

const statusLabel = (s: string): string =>
  ({ in_progress: 'In progress', paused: 'Paused', cancelled: 'Cancelled', completed: 'Completed' }[s] || s)

interface ProfileViewProps {
  client: ClientPayload
}

function ProfileView({ client }: ProfileViewProps): React.ReactElement {
...
export default function ClientDashboard({ setView }: ClientDashboardProps): React.ReactElement | null {
  ...
  useEffect(() => {
    ...
    const fetchProjects = () => {
      fetch('/api/client-projects', {
        ...
        .catch((err: Error) => logger.error('Failed to load client projects', { message: err.message }))
