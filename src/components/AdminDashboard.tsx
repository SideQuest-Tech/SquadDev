...
const crmServiceLabels: Record<string, string> = { website: 'Website', app: 'Mobile App', software: 'Custom Software', automation: 'Automation', existing: 'System Maintenance', mvp: 'MVP', unsure: 'Not Sure Yet', design: 'UI/UX Design', consulting: 'Technical Consulting' }
const serviceLabel = (value: string): string => crmServiceLabels[value] || getServiceLabel(value)

const slug = (value: any): string => String(value || '').toLowerCase().replace(/\s+/g, '-')

const budgetValue = (budget: any): number => {
...
const money = (value: number): string => new Intl.NumberFormat('en-ZA', {
  style: 'currency', currency: 'ZAR', maximumFractionDigits: 0
}).format(value || 0)

const dateLabel = (value: any): string => {
...
const makeDemoRequest = (): ProjectRequest => {
...
function Brand({ onClick }: BrandProps): React.ReactElement {
...
function StatusBadge({ value }: StatusBadgeProps): React.ReactElement {
  return <span className={`crm-badge status-${slug(value)}`}>{value}</span>
}

interface UrgencyBadgeProps { value?: string }
function UrgencyBadge({ value }: UrgencyBadgeProps): React.ReactElement {
  return <span className={`crm-badge urgency-${slug(value)}`}>{value || 'Medium'}</span>
}

interface MetricCardProps { icon: LucideIcon; label: string; value: any; note: string; tone: string }
function MetricCard({ icon: Icon, label, value, note, tone }: MetricCardProps): React.ReactElement {
...
type AdminView = 'overview' | 'requests' | 'pipeline' | 'analytics' | 'settings' | 'clients' | 'projects' | 'meetings'
...
function ViewHeader({ view, requests, onWebsite, onExport, onClearDemo }: ViewHeaderProps): React.ReactElement {
  ...
  const [title, desc] = copy[view] || ['Admin', '']
  const demoCount = requests.filter(request => request.isDemo).length
  return <header className="crm-header">
    <div><div className="eyebrow">SideQuest Tech admin</div><h1>{title}</h1><p>{desc}</p></div>
...
interface DashboardMetricsProps { requests: ProjectRequest[] }
function DashboardMetrics({ requests }: DashboardMetricsProps): React.ReactElement {
  ...
  const metrics: [LucideIcon, string, any, string, string][] = [
...
function EmptyState({ filtered, onWebsite, onDemo, onReset }: EmptyStateProps): React.ReactElement {
...
interface RequestActionsProps {
  ...
  onView: (r: ProjectRequest) => void
  onStatus: (id: number, status: string) => void
  onDelete: (id: number) => void
}
function RequestActions({ request, onView, onStatus, onDelete }: RequestActionsProps): React.ReactElement {
...
function RequestsView({ requests, filtered, filters, setFilter, onReset, onWebsite, onDemo, onView, onStatus, onDelete }: RequestsViewProps): React.ReactElement {
...
function OverviewView({ requests, onView, onNavigate, onWebsite, onDemo }: OverviewViewProps): React.ReactElement {
  ...
  return <div className="overview-grid">
    <section className="crm-panel overview-recent"><div className="section-title"><div><h2>Recent requests</h2><p>Newest opportunities submitted to SideQuest Tech.</p></div><button onClick={() => onNavigate('requests')}>View all <ChevronRight /></button></div>
      <div className="recent-list">{recent.map(request => <button key={request.id} onClick={() => onView(request)}><span className="client-avatar">{request.fullName.split(' ').map((part: string) => part[0]).join('').slice(0, 2)}</span><span><strong>{request.fullName}</strong><small>{request.company || serviceLabel(request.need)}</small></span><span><StatusBadge value={request.status} /><small>{dateLabel(request.createdAt)}</small></span></button>)}</div>
    </section>
...
function PipelineView({ requests, onView, onStatus }: PipelineViewProps): React.ReactElement {
...
function AnalyticsGroup({ title, icon: Icon, items, total }: AnalyticsGroupProps): React.ReactElement {
...
interface AnalyticsViewProps { requests: ProjectRequest[] }
function AnalyticsView({ requests }: AnalyticsViewProps): React.ReactElement {
  const serviceItems: [string, number][] = needOptions.map(([value]) => [serviceLabel(value).replace('I need ', ''), requests.filter(r => r.need === value).length]).filter(([, value]) => value) as [string, number][]
  const urgencyItems: [string, number][] = urgencyOptions.map(value => [value, requests.filter(r => r.urgency === value).length])
  const statusItems: [string, number][] = statusOptions.map(value => [value, requests.filter(r => r.status === value).length])
...
interface DetailRowProps { label: string; value?: any }
...
function ApprovalModal({ request, onClose, onApproved, showToast }: ApprovalModalProps): React.ReactElement {
...
function ClientsView({ adminSecret, showToast }: ClientsViewProps): React.ReactElement {
  ...
  const doAction = async (action: string, email: string, extra: Record<string, any> = {}) => {
...
function ProjectsView({ adminSecret, showToast }: ProjectsViewProps): React.ReactElement {
  ...
  const updateStatus = async (listId: string, newStatus: string, reason?: string) => {
    setActionLoading(true)
    try {
  ...
  const projectStatusLabel: Record<string, string> = { in_progress: 'In Progress', paused: 'Paused', cancelled: 'Cancelled', completed: 'Completed' }
  ...
  return <>
    <div className="kanban-board" style={{ gridTemplateColumns: 'repeat(4,minmax(210px,1fr))' }}>
      {columns.map(col => {
        ...
        const badgeClass: Record<string, string> = { in_progress: 'status-in-progress', paused: 'status-reviewing', cancelled: 'status-archived', completed: 'status-completed' }
        return <section className="kanban-column" key={col}>
          <header><span className={`crm-badge ${badgeClass[col]}`}>{projectStatusLabel[col]}</span><span>{cards.length}</span></header>
          <div>
...
function MeetingsView({ adminSecret, showToast }: MeetingsViewProps): React.ReactElement {
  ...
  const [actionLoading, setActionLoading] = useState(false)
  const [newMtg, setNewMtg] = useState<{ clientEmail: string; clientName: string } | null>(null)
  ...
  const respond = async (meeting: AdminMeeting, action: string) => {
  ...
  const fmtDT = (iso: string): string => new Date(iso).toLocaleString('en-ZA', { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', timeZone: 'Africa/Johannesburg' })
  ...
  const MeetingRow = ({ m }: { m: AdminMeeting }): React.ReactElement => {
...
interface RequestDrawerProps {
  ...
  onClose: () => void
  onStatus: (id: number, status: string) => void
  onNotes: (id: number, adminNotes: string) => void
  onDelete: (id: number) => void
  onCopy: (value: string, message: string) => void
  onApprove: (request: ProjectRequest) => void
}

function RequestDrawer({ request, onClose, onStatus, onNotes, onDelete, onCopy, onApprove }: RequestDrawerProps): React.ReactElement {
  ...
  return <div className="crm-drawer-overlay" onMouseDown={event => event.target === event.currentTarget && onClose()}><aside className="crm-drawer" aria-label={`Project request ${request.reference}`}>
    ...
    <div className="drawer-client"><span className="client-avatar large">{request.fullName.split(' ').map((part: string) => part[0]).join('').slice(0, 2)}</span><div><h3>{request.fullName}</h3><p>{request.company || 'Independent project'}</p></div><UrgencyBadge value={request.urgency} /></div>
...
export default function AdminDashboard({ onLogout, onClose }: AdminDashboardProps): React.ReactElement {
  ...
  useEffect(() => {
    ...
    fetch('/api/requests', { headers })
      .then(r => r.json())
      .then(async data => {
        let loaded: ProjectRequest[] = data.requests || []
        ...
        for (const key of legacyKeys) {
          try {
            ...
            await Promise.all(legacy.map((r: any) => fetch('/api/requests', {
              method: 'POST',
  ...
  const persist = (next: ProjectRequest[]) => {
    setRequests(next)
    fetch('/api/requests', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'x-admin-secret': adminSecret },
  ...
  const updateRequest = (id: number, patch: Partial<ProjectRequest>) => persist(requests.map(request => request.id === id ? { ...request, ...patch } : request))
  const updateStatus = (id: number, status: string) => updateRequest(id, { status })
  ...
  const addDemo = () => {
    ...
    }).catch((err: Error) => logger.error('Failed to persist demo request', { message: err.message }))
  }
  ...
  const filtered = useMemo(() => {
    ...
    return [...result].sort((a, b) => filters.sort === 'oldest' ? new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime() : filters.sort === 'budget' ? budgetValue(b.budget) - budgetValue(a.budget) : filters.sort === 'urgency' ? (urgencyRank[b.urgency ?? ''] ?? 0) - (urgencyRank[a.urgency ?? ''] ?? 0) : filters.sort === 'status' ? statusRank[a.status] - statusRank[b.status] : new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
  }, [requests, filters])

  const showMetrics = !['settings', 'clients', 'projects', 'meetings'].includes(activeView)
  const navigation: [LucideIcon, AdminView, string][] = [
